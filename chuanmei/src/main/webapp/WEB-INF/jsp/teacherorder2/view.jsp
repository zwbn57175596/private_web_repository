                	...
                	<input type="text" readonly="readonly" id="other_exp"
						...
						<tr>
							<td colspan="3"><font color="red"><b id="stu_tips">
										<c:choose>
											<c:when test="${teacherOrders.haveGroup == 1}">组长学号(多个之间用\",\"隔开):</c:when>
											<c:otherwise>学生学号(多个之间用","隔开):</c:otherwise>
										</c:choose>
								</b> </font> <input value="${teacherOrders.studentIds}" class="input-title"
								type="text" id="studentIds" name="studentIds" size="60"
								readonly="readonly" /></td>
							<td>是否分组实习<input type="checkbox" name="haveGroup"
								id="haveGroup" value="1" disabled="disabled"
